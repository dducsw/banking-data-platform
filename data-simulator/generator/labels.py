...
def generate_feature_snapshots(
    ...
    base_df = eligible_labels.join(
        customers.select(["customer_id", "customer_since", "annual_income"]),
        on="customer_id",
        how="left",
    )
    ...
    if "customer_id" not in loan_monthly_snapshot.columns:
        loan_snapshots = loan_monthly_snapshot.join(
            loans.select(["loan_id", "customer_id"]), on="loan_id", how="left"
        )
    else:
    ...
    comp_6m_df = (
        cust_months.join(complaints, on="customer_id", how="inner")
        .filter(
    ...
    unres_df = (
        cust_months.join(
            complaints.filter(~pl.col("resolved_flag")),
            on="customer_id",
            how="inner",
        )
    ...
    nps_df = (
        cust_months.join(
            feedback.filter(pl.col("nps_score").is_not_null()),
            on="customer_id",
            how="inner",
        )
        .filter(
