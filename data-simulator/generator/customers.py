...
from generator.locations import sample_locations
...
def generate_customers(
    ...
    """Generates the static customers DataFrame from the spine.
    ...
    # Sample locations in bulk
    locs = sample_locations(n, rng)
    ...
    for idx, (cid, pers_name) in enumerate(zip(customer_ids, personas)):
        ...
        # Recovered logic
        if age < 25:
            marital = rng.choice(["Single", "Married"], p=[0.9, 0.1])
        elif age > 60:
            marital = rng.choice(["Married", "Divorced", "Widowed"], p=[0.5, 0.2, 0.3])
        else:
            marital = rng.choice(["Single", "Married", "Divorced"], p=[0.3, 0.6, 0.1])
            
        if p_enum in salaried_personas:
            occupation = rng.choice(["Engineer", "Manager", "Teacher", "Doctor", "Accountant"])
            if p_enum == Persona.DIGITAL_NATIVE:
                income_bracket = "$50k - $100k"
            elif p_enum == Persona.AFFLUENT_MULTI_PRODUCT:
                income_bracket = "> $250k"
            else:
                income_bracket = rng.choice(["$50k - $100k", "$100k - $250k"])
        elif p_enum == Persona.DORMANT_WEALTHY:
            occupation = rng.choice(["Retired", "Business Owner"])
            income_bracket = "> $250k"
        else:
            occupation = rng.choice(["Student", "Service Worker", "Unemployed"])
            income_bracket = "< $50k"

        address = fake.street_address()
        email = f"{first_name.lower()}.{last_name.lower()}{cid}@example.com"
        phone = fake.phone_number()
        created_at = fake.date_between(start_date="-5y", end_date=config.sim_start)
        ...
        rows.append(
            {
                ...
                "employment_type": "Salaried" if p_enum in salaried_personas else "Self-Employed",
                "annual_income": float(rng.uniform(30000, 150000)),
                "country": "USA",
                "kyc_status": "Verified",
                "is_active": True,
                "cif_number": f"CIF{cid:08d}",
                "address": address,
                "city": loc["city"],
                "state": loc["state"],
                "zipcode": loc["zip"],
