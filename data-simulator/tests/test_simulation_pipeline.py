...
def test_no_future_activity_post_churn(sim_results):
    ...
    txn_fact = results["transactions"]
    login_fact = results["login_events"]

    for c in churned_custs:
        cid = c["customer_id"]
        c_month = c["churn_month"]

        # Transactions must not exceed churn month
        ...
        # Login events must not exceed churn month
        cust_logins = login_fact.filter(pl.col("customer_id") == cid)
        for row in cust_logins.to_dicts():
            assert row["login_month"] <= c_month, (
                f"Login event for customer {cid} found in month {row['login_month']} after churn month {c_month}!"
            )
...
def test_feature_non_leakage(sim_results):
    ...
    complaints_df = results["complaints"]
...
def test_behavioral_validations(sim_results):
    ...
    _, results = sim_results
    state_df = results["churn_simulation_state"]
    loan_pmt_df = results["loan_payments"]
    loan_master_df = results["loans"]
    ...
    if not loan_pmt_df.is_empty():
        ln_merged = loan_pmt_df.join(
...
def test_parquet_writer(sim_results):
    ...
    try:
        ...
        assert os.path.exists(os.path.join(test_out_dir, "customers.parquet"))
        assert os.path.exists(os.path.join(test_out_dir, "accounts.parquet"))

        # Verify partitioned directory exists
        acc_snap_dir = os.path.join(test_out_dir, "account_balance_snapshots")
...
def test_parallel_simulation_equivalence():
    ...
    cust_ids = res_par["customers"]["customer_id"].to_list()
    assert len(cust_ids) == len(set(cust_ids)), (
        "Duplicate customer_ids found in customers!"
    )

    acc_ids = res_par["accounts"]["account_id"].to_list()
    assert len(acc_ids) == len(set(acc_ids)), (
        "Duplicate account_ids found in accounts!"
    )

    card_ids = res_par["cards"]["card_id"].to_list()
    assert len(card_ids) == len(set(card_ids)), (
        "Duplicate card_ids found in cards!"
    )

    loan_ids = res_par["loans"]["loan_id"].to_list()
    assert len(loan_ids) == len(set(loan_ids)), (
        "Duplicate loan_ids found in loans!"
    )

    if not res_par["transactions"].is_empty():
        txn_ids = res_par["transactions"]["transaction_id"].to_list()
        assert len(txn_ids) == len(set(txn_ids)), (
            "Duplicate transaction_ids found in transactions!"
        )

    if not res_par["complaints"].is_empty():
        comp_ids = res_par["complaints"]["complaint_id"].to_list()
        assert len(comp_ids) == len(set(comp_ids)), (
            "Duplicate complaint_ids found in complaints!"
        )

    if not res_par["feedback"].is_empty():
        feed_ids = res_par["feedback"]["feedback_id"].to_list()
        assert len(feed_ids) == len(set(feed_ids)), (
            "Duplicate feedback_ids found in feedback!"
        )
    ...
    assert_fk_exists(
        res_par["accounts"], "customer_id", cust_ids_set, "accounts"
    )
    assert_fk_exists(
        res_par["cards"], "customer_id", cust_ids_set, "cards"
    )
    assert_fk_exists(res_par["loans"], "customer_id", cust_ids_set, "loans")
    assert_fk_exists(
        res_par["transactions"], "customer_id", cust_ids_set, "transactions"
    )
    assert_fk_exists(
        res_par["transactions"], "account_id", acc_ids_set, "transactions"
    )
    assert_fk_exists(
        res_par["account_ledger"], "customer_id", cust_ids_set, "account_ledger"
    )
    assert_fk_exists(
        res_par["account_balance_snapshots"],
        "account_id",
        acc_ids_set,
        "account_balance_snapshots",
    )
    assert_fk_exists(
        res_par["login_events"],
        "customer_id",
        cust_ids_set,
        "login_events",
    )
    assert_fk_exists(
        res_par["notifications"],
        "customer_id",
        cust_ids_set,
        "notifications",
    )
    assert_fk_exists(
        res_par["loan_payments"],
        "customer_id",
        cust_ids_set,
        "loan_payments",
    )
    ...
    assert_fk_exists(
        res_par["feedback"], "customer_id", cust_ids_set, "feedback"
    )
    assert_fk_exists(
