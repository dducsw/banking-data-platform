...
def check_fk_integrity(data_dir: str) -> bool:
    ...
    try:
        # Load tables
        cust = pl.read_parquet(os.path.join(data_dir, "customers.parquet"))
        ...
        # accounts
        acc = pl.read_parquet(os.path.join(data_dir, "accounts.parquet"))
        assert_fk(acc, "customer_id", cust_ids, "accounts")
        ...
        # cards
        card = pl.read_parquet(os.path.join(data_dir, "cards.parquet"))
        assert_fk(card, "customer_id", cust_ids, "cards")
        ...
        # loans
        loan = pl.read_parquet(os.path.join(data_dir, "loans.parquet"))
        assert_fk(loan, "customer_id", cust_ids, "loans")
        ...
        # transactions
        txn = pl.read_parquet(os.path.join(data_dir, "transactions/**/*.parquet"))
        assert_fk(txn, "customer_id", cust_ids, "transactions")
        assert_fk(txn, "account_id", acc_ids, "transactions")
        ...
        assert_fk(comp, "customer_id", cust_ids, "complaints")

        # feedback
        feed = pl.read_parquet(os.path.join(data_dir, "feedback/**/*.parquet"))
        assert_fk(feed, "customer_id", cust_ids, "feedback")
...
def main():
    ...
    # 1. Load Data
    cust_base = pl.read_parquet(os.path.join(baseline_dir, "customers.parquet"))
    cust_opt = pl.read_parquet(os.path.join(optimized_dir, "customers.parquet"))
    ...
    txn_base_df = pl.read_parquet(
        os.path.join(baseline_dir, "transactions/**/*.parquet")
    )
    txn_opt_df = pl.read_parquet(
        os.path.join(optimized_dir, "transactions/**/*.parquet")
    )
